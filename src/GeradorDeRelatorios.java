...
import filters.CategoriaIgualFilter;
import filters.IFilterStrategy;
import filters.NoFilter;
import filters.PrecoIntervalarFiltro;
import formatters.BoldFormatter;
import formatters.ColorFormatter;
...
import sort.ProductInsertionSort;
import sort.ProductQuickSort;
import sort.ProductSortingStrategy;
...
public class GeradorDeRelatorios {
	...
	private IFilterStrategy filtro;
	private Produto[] produtos;
	private ProductSortingStrategy algoritmo;
	private ProductComparator criterio;
	private HTMLFormatter[] formatters;

	public GeradorDeRelatorios(Produto[] produtos, ProductSortingStrategy algoritmo, ProductComparator criterio,
			IFilterStrategy filtro, HTMLFormatter... formatters) {
	...
	private void ordena(int ini, int fim) {
		algoritmo.sort(produtos, criterio, ini, fim);
	}
	...
	public void geraRelatorio(String arquivoSaida) throws IOException {
		...
		for (int i = 0; i < produtos.length; i++) {
			...
			if (filtro.isSelected(p)) {

				for (HTMLFormatter decorator : formatters) {
					p = decorator.decorate(p);
				}

				out.println(String.format(" <li> %s </li>", p.formataParaImpressao()));
	...
	public static Produto[] carregaProdutos() {
		...
		return new Produto[] { new ProdutoPadrao(1, "O Hobbit", "Livros", 2, 34.90),
	...
	public static void main(String[] args) {
		...
		HTMLFormatter[] formatters = new HTMLFormatter[]{
			new BoldFormatter(),
			new ColorFormatter("red"),
			new UnderlineFormatter()
		};

		gdr = new GeradorDeRelatorios(produtos, new ProductQuickSort(), ProductComparator.ESTOQUE_CRESC,
				new CategoriaIgualFilter("Games"), formatters);

		try {
			gdr.geraRelatorio("relatorioDecorado.html");
		} catch (IOException e) {
